...
PostProcessor::PostProcessor(Shader shader, uint32_t width, uint32_t height)
	: PostProcessoringShader(shader), Texture(), Width(width), Height(height), Confuse(false), Chaos(false), Shake(false)
{
	...
	this->initRenderData();
	this->PostProcessoringShader.SetInteger("scene", 0, true);
	...
	glUniform2fv(glGetUniformLocation(this->PostProcessoringShader.GetID(), "offsets"), 9, (float*)offsets);
	int edge_kernel[9] = {
		-1, -1, -1,
		-1,  8, -1,
		-1, -1, -1
	};
	glUniform1iv(glGetUniformLocation(this->PostProcessoringShader.GetID(), "edge_kernel"), 9, edge_kernel);
	float blur_kernel[9] = {
		1.0f / 16.0f, 2.0f / 16.0f, 1.0f / 16.0f,
		2.0f / 16.0f, 4.0f / 16.0f, 2.0f / 16.0f,
		1.0f / 16.0f, 2.0f / 16.0f, 1.0f / 16.0f
	};
	glUniform1fv(glGetUniformLocation(this->PostProcessoringShader.GetID(), "blur_kernel"), 9, blur_kernel);
}
...
void PostProcessor::Render(float time)
{
	...
	this->PostProcessoringShader.Use();
	this->PostProcessoringShader.SetFloat("time", time);
	this->PostProcessoringShader.SetInteger("confuse", this->Confuse);
	this->PostProcessoringShader.SetInteger("chaos", this->Chaos);
	this->PostProcessoringShader.SetInteger("shake", this->Shake);
